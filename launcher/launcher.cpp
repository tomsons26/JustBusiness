...
#define EXE_ENTRY 0x0042E450 // WinMain
//#define EXE_ENTRY 0x00401000    // real entry point

// Define the EXE to launch and the DLL to inject.
#define EXE_NAME "monopoly.exe" // acutual binary (Original disc version only supported).
#define DLL_NAME "justbusiness.dll" // Our reimplementation DLL.

// Define the hash to compare against.
#define EXE_HASH "18648049a0a8d2dc70d513a7d8f1ba7cbbf23c4b"
...
void Check_Hash()
{
    ...
    if (strcmp(EXE_HASH, hash) != 0) {
        ...
        MessageBoxA(NULL,
            "Your " EXE_NAME " hash does not match expected.\nCheck you have the correct version.",
            "Hash mismatch!",
...
void Inject_Loader(const char *path, const char *dllname, char *args)
{
    ...
    if (CreateProcessA(path, args, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, NULL, &StartupInfo, &ProcessInformation)) {
        ...
        try {
            ...
            //MessageBox(NULL, "Attach debugger or continue.", "Debug Time!", MB_OK | MB_SERVICE_NOTIFICATION);
